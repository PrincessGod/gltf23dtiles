...
var mergeBuffers = gltfPipeline.mergeBuffers;
var loadGltfUris = gltfPipeline.loadGltfUris;
var removePipelineExtras = gltfPipeline.removePipelineExtras;
...
function convertToBinaryGltf(gltf) {
    ...
    return loadGltfUris(gltf)
        .then(function (gltf) {
            mergeBuffers(gltf);
            var buffersBinary = gltf.buffers[0].extras._pipeline.source;
            removePipelineExtras(gltf);
            replaceBuffers(gltf);
            // mergeBuffersUri(gltf,buffersBinary);
            return gltfToGlb(gltf, buffersBinary);
        });

}

function replaceBuffers(gltf) {
...
function mergeBuffersUri(gltf, binaryBuffer) {
    var buffer = gltf.buffers[0];
    buffer.uri = 'data:application/octet-stream;base64,' + binaryBuffer.toString('base64');
}
